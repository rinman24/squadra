...
The PR2 direction folds these into a single argparse-native `flotilla` surface
(`flotilla init | tick | start | stop | status | log | slice …`); the per-module
`python -m flotilla.supervisor` / `flotilla.status` entry points stay as internal
seams.
