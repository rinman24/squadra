...
- `fleet-ado-pat` — the Azure DevOps PAT, unchanged: fetched at tick time
  (`flotilla.secrets`) for the gswa-backend clone (`FLEET_HOME` / `FLEET_APP_REPO_URL`)
...
gswa-backend and the board stay on Azure DevOps; only flotilla's source host moved.
