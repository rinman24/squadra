...
from flotilla.domain import Finalized, FinalizeOutcome, Lifecycle, Reaped, ReapOutcome
from flotilla.status import FleetStatus, load, write
from flotilla.supervisor import TickSeams, finalize_pass, reap_pass, run_tick
...
def test_finalize_retires_a_merged_done_slice(
    ...
    outcome: FinalizeOutcome = finalize_pass(make_seams(), make_config())
...
def test_finalize_derives_the_branch_when_no_status_file_exists(
    ...
    make_issue(41, title="feat: example", state=Lifecycle.DONE, tags=["fleet:claimed"])
    fake_board.completed_prs["feat/slice-41-example"] = "https://pr/41"
    outcome: FinalizeOutcome = finalize_pass(make_seams(), make_config())
    assert outcome.finalized == (41,)
    assert fake_cleaner.cleaned == ["feat/slice-41-example"]
...
def test_reap_requeues_a_stale_dead_runner_and_archives_the_worktree(
    ...
    outcome: ReapOutcome = reap_pass(seams, config, now=_NOW)
    ...
    assert ["git", "-C", str(config.fleet_home), "worktree", "prune"] in git_runner.calls
...
def test_reap_leaves_fresh_runners_alone(
    ...
    outcome: ReapOutcome = reap_pass(seams, make_config(), now=_NOW)
...
def test_reap_never_touches_parked_runners(
    ...
    make_issue(41, state=Lifecycle.ACTIVE, tags=["fleet:claimed", "fleet:needs-decision"])
    write(
...
def test_reap_escalates_when_the_attempt_cap_is_exhausted(
    ...
    assert fake_board.item_state(41) == Lifecycle.ACTIVE  # stays parked-failed, not requeued
...
def test_reap_skips_a_deliberate_park_with_tag_even_when_stale_and_dead(
    ...
    make_issue(41, state=Lifecycle.ACTIVE, tags=["fleet:claimed", "fleet:awaiting-pr-approval"])
    write(
