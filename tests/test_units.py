...
def _ctx() -> UnitContext:
    return UnitContext(
        ...
        fleet_home=Path("/opt/flotilla/gswa-backend"),
...
def test_service_is_oneshot_running_fleet_tick() -> None:
    ...
    assert "WorkingDirectory=/opt/flotilla/gswa-backend" in service


def test_service_passes_runtime_env_but_no_secret() -> None:
    service: str = render_unit("flotilla.service", _ctx())
    assert "Environment=FLEET_KEY_VAULT=gswa-fleet-kv-07c17c" in service
    assert "Environment=FLEET_HOME=/opt/flotilla/gswa-backend" in service
