...
def finalize_pass(seams: TickSeams, config: FlotillaConfig) -> FinalizeOutcome:
    ...
    status file only provides the branch fast-path.
    """
...
class TmuxLauncher:
    ...
    path — plain tmux, not the experimental agent-teams
...
class ClaudeCleanup:
    ...
    existing worktree hooks; its Step 3b sweep is
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="One AFK-fleet supervisor tick (ADR-0001).",
    )
